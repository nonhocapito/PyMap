...
def compose_final(
    ...
    credits_bbox = draw.textbbox((0, 0), credits, font=font_info)
    credits_w = credits_bbox[2] - credits_bbox[0]
    credits_x = canvas_w - margin_px - credits_w
    draw.text( (credits_x, bar_y), credits, fill="#666666", font=font_info)
...
def main():
    ...
    try:
        # Input coordinate
        lat = float(input("Insert LATITUDE (es. 40.167): "))
        lon = float(input("Insert LONGITUDE (es. 18.017): ")) 
        
        # Input scala
        scale_str = input("Select SCALE (es. 25000 for 1:25.000) [default 25000]: ")
        scale = int(scale_str) if scale_str.strip() else 25_000
        
        # Titolo
        title = input("Insert TITLE for the map: ")
        if not title.strip():
            title = f"Map {lat}N {lon}E"
            
        # Formato foglio
        print(f"Supported formats: {', '.join(PAPER_SIZES.keys())}")
        paper = input("Sheet format (es. A4, A3) [default A4]: ").upper()
        if not paper or paper not in PAPER_SIZES:
            paper = "A4"
            
        # Orientamento
        land_input = input("Landscape ORIENTATION? (Y/n) [default n]: ").lower()
        landscape = True if land_input == 'y' else False
        
        # DPI (opzionale, lo teniamo fisso a 150 per bilanciare qualità e velocità)
        try:
            dpi = int(input("Insert DPI [default 150 dpi]: "))
        except:
            dpi=150
